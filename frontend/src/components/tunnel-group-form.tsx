...
import { api, type TunnelGroup, type TunnelGroupWritePayload } from "@/lib/api"
import {
  FormFooter,
  FormText,
  FormTextarea,
...
export function TunnelGroupForm({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit)
        return api<TunnelGroup>(`/api/tunnel-groups/${item!.id}/`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        })
      return api<TunnelGroup>("/api/tunnel-groups/", {
        method: "POST",
        body: JSON.stringify(payload),
      })
    },
