...
export interface TunnelGroupBulkBarProps {
  selected: TunnelGroup[]
  onCleared: () => void
}

export function TunnelGroupBulkBar({
  selected,
  onCleared,
}: TunnelGroupBulkBarProps) {
  ...
  return (
    <>
      ...
      <BulkDeleteConfirm
        ids={ids}
        sample={selected.slice(0, 5).map((g) => g.name)}
        open={deleteOpen}
...
function BulkDeleteConfirm({
  ...
  const m = useMutation({
    mutationFn: () =>
      Promise.all(
        ids.map((id) =>
          api<void>(`/api/tunnel-groups/${id}/`, { method: "DELETE" })
        )
      ),
    onSuccess: () => {
      toast.success(
        `Deleted ${ids.length} tunnel group${ids.length === 1 ? "" : "s"}.`
      )
      onOpenChange(false)
      onDone()
    },
    onError: (err) => apiErrorToast(err),
    // Refresh the list even on partial failure: some DELETEs may have
    // succeeded server-side before another rejected.
    onSettled: () => {
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Delete {ids.length} tunnel group{ids.length === 1 ? "" : "s"}?
          </AlertDialogTitle>
          <AlertDialogDescription>
