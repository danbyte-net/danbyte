...
export interface ContactRoleDeleteDialogProps {
  item: ContactRole | null
  onOpenChange: (open: boolean) => void
  onDeleted?: () => void
}

export function ContactRoleDeleteDialog({
  item,
  onOpenChange,
  onDeleted,
}: ContactRoleDeleteDialogProps) {
  const qc = useQueryClient()
  const blocked = !!item && item.assignment_count > 0
  const m = useMutation({
    mutationFn: () =>
      api<void>(`/api/contact-roles/${item!.id}/`, { method: "DELETE" }),
    onSuccess: () => {
