...
export interface VlanBulkBarProps {
  selected: VLAN[]
  onCleared: () => void
}

export function VlanBulkBar({ selected, onCleared }: VlanBulkBarProps) {
  ...
  return (
    <>
      <div className="pointer-events-none fixed inset-x-0 bottom-4 z-40 flex justify-center">
        <div className="pointer-events-auto flex items-center gap-2 rounded-lg border border-border bg-popover px-2 py-1.5 text-popover-foreground shadow-lg">
          ...
          <Button size="sm" variant="ghost" className="h-7 px-2" asChild>
            <Link to="/vlans/bulk-edit" search={{ ids: ids.join(",") }}>
              <Pencil className="mr-1 h-3 w-3" /> Edit
            </Link>
          </Button>
          <BulkExport ioType="vlan" ids={ids} />
          <Button
            size="sm"
            variant="ghost"
      ...
      <BulkDeleteConfirm
        ids={ids}
        sample={selected.slice(0, 5).map((v) => `${v.vlan_id} ${v.name}`)}
        open={deleteOpen}
...
function BulkDeleteConfirm({
  ...
  const m = useMutation({
    mutationFn: () =>
      api<{ deleted: number }>("/api/vlans/bulk-delete/", {
        method: "POST",
        body: JSON.stringify({ ids }),
      }),
    onSuccess: (res) => {
      toast.success(
        `Deleted ${res.deleted} VLAN${res.deleted === 1 ? "" : "s"}.`
      )
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Delete {ids.length} VLAN{ids.length === 1 ? "" : "s"}?
          </AlertDialogTitle>
          <AlertDialogDescription>
