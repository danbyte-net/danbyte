...
def _resolve_fk(field, value, tenant):
...
def _coerce(field, value, tenant):
    if value is None:
        return None
    ...
    if field.is_relation:
        return _resolve_fk(field, value, tenant)
...
def _build(model, tenant, row, fields):
    obj = model()
...
def import_rows(model, tenant, rows, *, dry_run=False) -> dict:
    fields = {f.name: f for f in _importable_fields(model)}
    created, errors = 0, []
    for i, row in enumerate(rows):
        try:
            with transaction.atomic():
                obj = _build(model, tenant, row, fields)
