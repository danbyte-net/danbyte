...
- **IPAM → DHCP / DNS** (clusters inside the IPAM section — scopes,
  ...
  the synced data*, aggregated across every server: **DHCP** → Scopes,
  Reservations, Leases; **DNS** → Zones, Records. Each list filters by server
  and links back to the prefixes and IP addresses the data maps to. These pages
  appear once the matching toggle is on.
